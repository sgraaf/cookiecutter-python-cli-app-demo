...
Thank you for your interest in contributing to *Cookiecutter Python CLI App Demo*!
...
To start developing *Cookiecutter Python CLI App Demo*, follow these instructions to setup your development environment:

1. Create a *fork* (your own personal copy) of the *Cookiecutter Python CLI App Demo* repository via https://github.com/sgraaf/cookiecutter-python-cli-app-demo/fork.

1. Clone the repository by replacing `<YOUR-USERNAME>` with your GitHub username:

   ```shell
   git clone https://github.com/<YOUR-USERNAME>/cookiecutter-python-cli-app-demo.git
...
*Cookiecutter Python CLI App Demo* adheres to [the Black code style](https://black.readthedocs.io/en/stable/the_black_code_style/current_style.html) and uses [Ruff](https://docs.astral.sh/ruff/) for linting and formatting code. Make sure your code is formatted properly:
...
*Cookiecutter Python CLI App Demo* has a comprehensive test suite, which includes smoke tests, unit tests and integration tests, and uses [pytest](https://docs.pytest.org/en/stable/) to execute it:
...
*Cookiecutter Python CLI App Demo* uses [Google Style Python Docstrings](https://www.sphinx-doc.org/en/master/usage/extensions/example_google.html) to document its API:
...
The CLI reference in `README.md` and `docs/cli.md` is generated from the actual `--help` output of *Cookiecutter Python CLI App Demo* with [cog](https://cog.readthedocs.io/en/latest/). If you change the command-line interface, regenerate it:
...
*Cookiecutter Python CLI App Demo* follows the [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) format for its changelog:
...
When your pull request is approved, it will be merged into the `main` branch. Your change will only be available to users the next time *Cookiecutter Python CLI App Demo* is released.
